...
**Web GUI** — an embedded HTTP server (`--web[=PORT]`, or the Web GUI toggle under Advanced in the F9 overlay) that serves the emulator to any browser on the network: live screen as a multipart GIF stream (in-tree encoder, no dependencies, ~25 fps with change detection so static screens cost nothing), full input capture — click the screen and the browser's keyboard **and mouse** (pointer lock, relative motion into the SYMBiFACE / Albireo / AMX pointer device, same dispatch as the emulator window) belong to the CPC until **Ctrl+Enter** releases them — plus a touch-friendly on-screen joystick, paste-text box, and reset button. Single-threaded and polled from the frame loop like every other subsystem; up to 8 simultaneous connections; slow viewers just receive fewer frames. Default port 1984. `--web` implies `--headless` (offscreen video, dummy audio): no window on the host, the browser is the only interface — while the overlay toggle and the `web_gui` config key serve the same stream with the host window still visible. For a permanent web appliance, a systemd user unit is installed as `1984-web.service`: `systemctl --user enable --now 1984-web` gives you a browser-only CPC (add `loginctl enable-linger` to survive logout / start at boot). **Security note: it binds `0.0.0.0` with no authentication — anyone on your network can watch and type. Enable it only on networks you trust.**
