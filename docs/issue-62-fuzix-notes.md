...
|---|---|---|
| `1a3393a` | FDC port decode now requires `lo bit 7 = 0` | Real FDC lives at `0xFB7E/0xFB7F`; we used to claim the whole `0xFB**` range. FUZIX's USIfAC probe at `0xFBD8` (`usifexists` in `Kernel/platform/platform-cpcsme/plt_ch375.h`) was reading the FDC main status register instead of `0xFF`, so the driver assumed a USIfAC was present and proceeded with the rest of its init protocol — producing spurious "Error configuring Usifac" output and dirtying the kernel boot log. With the lo-bit-7 gate in place, `IN A,(0xFBD8)` correctly returns `0xFF` (open bus) when no USIfAC is present, and the probe short-circuits. |
