...
- **Host directory vs. SymbOS storage.** Host-directory mode serves the M4 file API directly, so BASIC `CAT`/`LOAD`/`SAVE` see host changes live. SymbOS's M4 storage driver does not use that API: it mounts the card through raw `C_SDREAD`/`C_SDWRITE` sectors. A directory has no sector layout, so direct SymbOS storage requires a raw FAT `m4_image`. Both may be configured together: `m4_path` remains the live file-API backing while `m4_image` supplies the SymbOS block device.
